...
<header class="entete">
  <div class="entete__barre">
    ...
    <div class="entete__droite">
      ...
      <div class="resa">
        <button class="resa__btn btn btn--or" aria-expanded="false" aria-haspopup="true">Réserver</button>
      ...
      <?php if (!empty($item['sous_menu'])): ?>
        <div class="panneau__accordeon"<?= $actif ? ' data-ouvert' : '' ?>>
          ...
          <div class="panneau__sous">
            <a href="<?= url($item['url']) ?>">Découvrir</a>
            <?php foreach ($item['sous_menu'] as $sous): ?>
              <a href="<?= url($sous['url']) ?>"><?= e($sous['libelle']) ?></a>
            <?php endforeach; ?>
          </div>
        </div>
      <?php else: ?>
        <a class="panneau__lien" href="<?= url($item['url']) ?>"<?= $actif ? ' aria-current="page"' : '' ?>>
          <span class="panneau__num"><?= $num ?></span><?= e($item['libelle']) ?>
        </a>
      <?php endif; ?>
    <?php endforeach; ?>
