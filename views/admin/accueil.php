...
<form class="bo-form" method="post" action="<?= url('/admin/accueil') ?>" enctype="multipart/form-data">
  ...
  <fieldset>
    <legend>Bloc « Le cabinet »</legend>
    <div class="bo-rangee">
      <div class="bo-champ">
        <label for="a-csur">Sur-titre</label>
        <input id="a-csur" type="text" name="cabinet_surtitre" value="<?= e($accueil['cabinet']['surtitre']) ?>">
      </div>
      <div class="bo-champ">
        <label for="a-ctit">Titre</label>
        <input id="a-ctit" type="text" name="cabinet_titre" value="<?= e($accueil['cabinet']['titre']) ?>">
      </div>
    </div>
    <div class="bo-champ">
      <label for="a-ctex">Texte</label>
      <textarea id="a-ctex" name="cabinet_texte" rows="6"><?= e(implode("\n\n", $accueil['cabinet']['paragraphes'])) ?></textarea>
      <p class="bo-aide">Une ligne vide sépare deux paragraphes.</p>
    </div>
    <div class="bo-champ">
      <label for="a-cpts">Points forts</label>
      <textarea id="a-cpts" name="cabinet_points" rows="4"><?= e($points) ?></textarea>
      <p class="bo-aide">Un par ligne : <code>titre | texte</code>. La numérotation est automatique.</p>
    </div>
    <div class="bo-champ">
      <label>Photo</label>
      <?= $view->partial('admin/choix-photo', [
          'medias' => $medias, 'nom' => 'cabinet_image', 'id' => 'cab',
          'choisie' => $accueil['cabinet']['image'], 'vide' => '',
      ]) ?>
    </div>
  </fieldset>
