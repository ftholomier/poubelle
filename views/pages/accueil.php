...
<section class="heros heros--accueil">
  ...
  <div class="heros__contenu conteneur">
    ...
    <div class="heros__actions">
      <a class="btn btn--magenta" href="<?= lien($hero['bouton_principal']['url']) ?>">
  ...
  <div class="conteneur">
    <div class="duo">
      <div class="duo__media reveler">
        <img src="<?= image($page['cabinet']['image']) ?>"
             alt="<?= e($site['nom']) ?> — <?= e($page['cabinet']['titre']) ?>" loading="lazy">
      </div>

      <div class="duo__texte reveler">
        <p class="surtitre"><?= e($page['cabinet']['surtitre']) ?></p>
        <h2 class="titre-section"><?= e($page['cabinet']['titre']) ?></h2>
        <?php foreach ($page['cabinet']['paragraphes'] as $p): ?>
          <p><?= e($p) ?></p>
        <?php endforeach; ?>

        <ul class="points">
          <?php foreach ($page['cabinet']['points'] as $point): ?>
        ...
        <?php if (!empty($page['cabinet']['lien']['url'])): ?>
          <a class="lien-fleche" href="<?= lien($page['cabinet']['lien']['url']) ?>">
            <?= e($page['cabinet']['lien']['libelle']) ?>
          </a>
        <?php endif; ?>
      </div>
    </div>
