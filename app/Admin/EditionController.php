...
final class EditionController
{
    ...
    public function accueilEnvoi(): string
    {
        ...
        $a['cabinet']['surtitre']    = trim((string) ($_POST['cabinet_surtitre'] ?? ''));
        $a['cabinet']['titre']       = trim((string) ($_POST['cabinet_titre'] ?? ''));
        $a['cabinet']['paragraphes'] = self::paragraphes((string) ($_POST['cabinet_texte'] ?? ''));
        $a['cabinet']['image']       = $this->photoFacultative('cabinet_image', (string) $a['cabinet']['image']);

        $points = [];
        foreach (self::lignes((string) ($_POST['cabinet_points'] ?? '')) as $i => $ligne) {
    ...
    {
        return $this->view->render('admin/cabinet', [
            'page'    => ['titre' => 'Page « Le cabinet »'],
            'cabinet' => $this->content->load('pages/le-cabinet'),
            'medias'  => $this->mediatheque->lister(),
        ], 'admin/layout');
    }

    public function cabinetEnvoi(): string
    {
        if (!Csrf::verifier()) {
            return $this->rediriger('/admin/cabinet');
        }
        $c = $this->content->load('pages/le-cabinet');
        ...
        $this->content->save('pages/le-cabinet', $c);
        Session::flash('succes', 'Page « Le cabinet » enregistrée.');
        return $this->rediriger('/admin/cabinet');
    }
