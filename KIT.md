...
```json
{
  "intro": "…",
  "items": [
    { "slug": "lodge-a", "nom": "…", "image": "…", "actif": true, "meta": {} }
  ]
}
```
...
- Pages éditoriales, collections avec fiches, galerie filtrable, formulaire de
  contact avec piège à robots
...
3. **Collections** — `Seo::COLLECTIONS` associe une page à une collection.
   Adapter ou remplacer `hebergements` / `peche` selon le métier.
