...
<form class="bo-form" method="post" action="<?= url('/admin/societe') ?>" enctype="multipart/form-data">
  ...
  <fieldset>
    ...
    <div class="bo-rangee">
      <div class="bo-champ">
        <label for="c-pnom">Nom</label>
        <input id="c-pnom" type="text" name="portrait_nom" value="<?= e($societe['portrait']['nom'] ?? '') ?>">
      </div>
      <div class="bo-champ">
        <label for="c-prol">Fonction</label>
        <input id="c-prol" type="text" name="portrait_role" value="<?= e($societe['portrait']['role'] ?? '') ?>">
      </div>
    </div>
    <div class="bo-champ">
      <label for="c-pcit">Citation</label>
      <input id="c-pcit" type="text" name="portrait_citation" value="<?= e($societe['portrait']['citation'] ?? '') ?>">
      <p class="bo-aide">Sans guillemets : ils sont ajoutés à l’affichage.</p>
    </div>
    <div class="bo-champ">
      <label for="c-ptex">Texte</label>
      <textarea id="c-ptex" name="portrait_texte" rows="6"><?= e(implode("\n\n", (array) ($societe['portrait']['paragraphes'] ?? []))) ?></textarea>
      <p class="bo-aide">Une ligne vide sépare deux paragraphes.</p>
    </div>
    <div class="bo-champ">
      <label>Photo du portrait</label>
      <?= $view->partial('admin/choix-photo', [
          'medias' => $medias, 'nom' => 'portrait_image', 'id' => 'portrait',
          'choisie' => $societe['portrait']['image'] ?? '', 'vide' => '',
      ]) ?>
    </div>
  </fieldset>

  <fieldset>
    <legend>Repères chiffrés</legend>
    <div class="bo-champ">
      <label for="c-rep">Un repère par ligne</label>
      <textarea id="c-rep" name="reperes" rows="5"><?= e(implode("\n", array_map(
          static fn(array $r): string => trim(($r['valeur'] ?? '') . '|' . ($r['unite'] ?? '') . '|' . ($r['libelle'] ?? '')),
          (array) ($societe['reperes']['items'] ?? [])
      ))) ?></textarea>
      <p class="bo-aide">
        Format : <code>valeur|unité|libellé</code>. Par exemple
        <code>25|ans|D’expérience du paysage</code>. L’unité peut rester vide :
        <code>2012||Création de l’entreprise</code>.
      </p>
    </div>
  </fieldset>
