...
final class Seo
{
    ...
    public const PAGES = [
        ...
        'la-societe'       => ['slug' => 'a-propos',          'nom' => 'À propos'],
        'nos-services'     => ['slug' => 'nos-prestations',   'nom' => 'Nos prestations'],
        ...
        'devis'            => ['slug' => 'demander-un-devis', 'nom' => 'Demander un devis'],
    ...
    public const COLLECTIONS = ['nos-services' => 'services'];
    ...
    private const CONTENUS = [
        ...
        'pages/nos-valeurs', 'pages/faq', 'pages/devis', 'pages/contact',
    ...
    private function noeudDomaine(array $site, string $base, string $id, callable $media): array
    {
        ...
        $noeud = [
            // LandscapingBusiness décrit précisément le métier ; c'est un
            // LocalBusiness pour les moteurs, avec l'activité en plus.
        ...
        // Les établissements secondaires : un paysagiste implanté dans deux
        // communes est cherché depuis les deux, et une seule adresse
        // déclarée revient à en abandonner une.
        $autres = [];
        foreach ((array) ($site['adresses_secondaires'] ?? []) as $lieu) {
            $ville = trim((string) ($lieu['ville'] ?? ''));
            if ($ville === '') {
                continue;
            }
            $autres[] = [
                '@type'   => 'Place',
                'name'    => (string) ($site['nom'] ?? '') . ' — ' . $ville,
                'address' => [
                    '@type'           => 'PostalAddress',
                    'streetAddress'   => (string) ($lieu['rue'] ?? ''),
                    'postalCode'      => (string) ($lieu['cp'] ?? ''),
        ...
        if (($site['fondation']['annee'] ?? '') !== '') {
            $noeud['foundingDate'] = (string) $site['fondation']['annee'];
        }
        if (($site['fondation']['fondatrice'] ?? '') !== '') {
            $noeud['founder'] = ['@type' => 'Person', 'name' => (string) $site['fondation']['fondatrice']];
        }

        $horaires = $this->horairesStructures((string) ($site['contact']['horaires'] ?? ''));
        if ($horaires !== null) {
            $noeud['openingHoursSpecification'] = $horaires;
        }
        ...
        foreach ($this->content->publies('services') as $service) {
            if (($service['nom'] ?? '') !== '') {
                $prestations[] = [
                    ...
                    'itemOffered' => [
                        '@type'       => 'Service',
                        'name'        => (string) $service['nom'],
                        'description' => (string) ($service['resume'] ?? ''),
                    ],
                ];
            }
        }
     ...
     *
     * Le champ est saisi en clair dans le back-office (« Du lundi au vendredi,
     * de 8h30 à 18h00 ») : on n'en tire des données structurées que si la
     * phrase est reconnue, plutôt que de publier un horaire inventé.
     *
     * @return array<string, mixed>|null
     */
    private function horairesStructures(string $texte): ?array
    {
        if (preg_match('/lundi.{0,20}vendredi.*?(\d{1,2})\s*h\s*(\d{0,2}).*?(\d{1,2})\s*h\s*(\d{0,2})/ui', $texte, $m) !== 1) {
            return null;
        }

        $heure = static fn(string $h, string $m): string
            => str_pad($h, 2, '0', STR_PAD_LEFT) . ':' . str_pad($m !== '' ? $m : '0', 2, '0', STR_PAD_LEFT);

        return [
            '@type'     => 'OpeningHoursSpecification',
            'dayOfWeek' => ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'],
            'opens'     => $heure($m[1], $m[2]),
            'closes'    => $heure($m[3], $m[4]),
        ];
    }
    ...
    private function noeudFiche(string $cle, array $item, string $base, string $domaine, callable $media): array
    {
        ...
        return [
            ...
            'areaServed'  => ['@type' => 'AdministrativeArea', 'name' => 'Doubs et Territoire de Belfort'],
        ];
    }
