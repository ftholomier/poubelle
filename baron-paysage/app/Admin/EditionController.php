...
final class EditionController
{
    ...
    public function accueilEnvoi(): string
    {
        ...
        $a['citation']['texte']  = trim((string) ($_POST['citation_texte'] ?? ''));
        $a['citation']['auteur'] = trim((string) ($_POST['citation_auteur'] ?? ''));

        $a['realisations']['surtitre'] = trim((string) ($_POST['realisations_surtitre'] ?? ''));
        $a['realisations']['titre']    = trim((string) ($_POST['realisations_titre'] ?? ''));
        $a['realisations']['texte']    = trim((string) ($_POST['realisations_texte'] ?? ''));
    ...
    public function societeEnvoi(): string
    {
        ...
        $c['portrait']['nom']         = trim((string) ($_POST['portrait_nom'] ?? ''));
        $c['portrait']['role']        = trim((string) ($_POST['portrait_role'] ?? ''));
        $c['portrait']['citation']    = trim((string) ($_POST['portrait_citation'] ?? ''));
        $c['portrait']['paragraphes'] = self::paragraphes((string) ($_POST['portrait_texte'] ?? ''));
        $c['portrait']['image']       = $this->photoFacultative(
            'portrait_image',
            (string) ($c['portrait']['image'] ?? '')
        );

        // Les repères chiffrés sont saisis en clair, un par ligne, sous la
        // forme « 25|ans|D'expérience ». Quatre champs par repère auraient
        // fait seize champs à l'écran pour quatre nombres.
        if (isset($_POST['reperes'])) {
            $c['reperes']['items'] = self::reperes((string) $_POST['reperes']);
        }

        foreach (array_keys((array) ($c['piliers']['items'] ?? [])) as $i) {
        ...
        Session::flash('succes', 'Page « À propos » enregistrée.');
        return $this->rediriger('/admin/societe');
    }
    ...
    /**
     * Écran Réalisations : on entre par la catégorie, on coche les photos,
     * puis on légende celles qu'on a retenues.
     *
     * L'écran d'origine proposait un formulaire par photo — nom, catégorie,
     * légende, ordre, publication — soit une quarantaine de blocs à parcourir
     * pour ranger une image. Il a été ramené à une planche à cocher. La
     * légende y est revenue, mais à part : le client l'a demandée pour
     * chaque photo, et elle ne concerne que celles qui sont retenues.
     */
    public function realisations(): string
    {
        $galerie = $this->content->load('realisations');
        // Les catégories sont les prestations publiées, plus celles que le
        // contenu déclare lui-même (« Le chantier » n'a pas de page produit).
        $categories = PageController::categoriesGalerie($this->content);

        // catégorie affichée : celle demandée, sinon la première
        $courante = trim((string) ($_GET['gamme'] ?? ''));
        if ($courante === '' || !isset($categories[$courante])) {
            $courante = (string) (array_key_first($categories) ?? '');
        }
    ...
    public function realisationsEnvoi(): string
    {
        ...
        // vide sur le site.
        // Les légendes déjà saisies sont relues avant d'être réécrites :
        // décocher puis recocher une photo ne doit pas la perdre.
        $legendes = [];
        foreach ((array) ($galerie['gammes'][$gamme] ?? []) as $photo) {
            if (is_array($photo) && ($photo['image'] ?? '') !== '') {
                $legendes[(string) $photo['image']] = (string) ($photo['legende'] ?? '');
            }
        }
        foreach ((array) ($_POST['legendes'] ?? []) as $chemin => $texte) {
            $legendes[(string) $chemin] = trim((string) $texte);
        }
        ...
        foreach ((array) ($_POST['photos'] ?? []) as $chemin) {
            ...
            if (!$this->mediatheque->existe($chemin) || isset($vues[$chemin])) {
                continue;
            }
            $vues[$chemin] = true;
            $choisies[] = [
                'image'   => $chemin,
                'legende' => trim((string) ($legendes[$chemin] ?? '')),
            ];
        }
    ...
    {
        return $this->view->render('admin/devis', [
            'page'   => ['titre' => 'Page « Demander un devis »'],
            'devis'  => $this->content->load('pages/devis'),
            'medias' => $this->mediatheque->lister(),
        ], 'admin/layout');
    }
    ...
    {
        if (!Csrf::verifier()) {
            return $this->rediriger('/admin/devis');
        }
        $d = $this->content->load('pages/devis');
        ...
        foreach (array_keys((array) ($d['etapes'] ?? [])) as $i) {
            $d['etapes'][$i]['titre'] = trim((string) ($_POST['etape_titre_' . $i] ?? ''));
            $d['etapes'][$i]['texte'] = trim((string) ($_POST['etape_texte_' . $i] ?? ''));
        }
        ...
        if ($objets !== []) {
            $d['formulaire']['objets'] = $objets;
        }

        $this->content->save('pages/devis', $d);
        Session::flash('succes', 'Page « Demander un devis » enregistrée.');
        return $this->rediriger('/admin/devis');
    }
