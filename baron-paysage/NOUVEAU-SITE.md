...
```bash
cp -r baron-paysage/ nouveau-client/ && cd nouveau-client
rm -rf .git data/pages data/*.json data/admin storage/cache/* storage/sauvegardes/*
git init && git add -A && git commit -m "Socle repris du site Baron Paysage"
...
```
Seo::PAGES        une entrée : clé, slug, titre, description
data-modele/pages/<clé>.json   le contenu
app/routes.php    une route
views/pages/      une vue
app/Admin/        un écran d'édition + sa route dans routes-admin.php
```
...
ton, et quand ramener vers un devis.
...
Trois scripts dans `outils/verifs/`, qui sortent en code 1 s'ils trouvent
...
| `app/Admin/` sauf les écrans propres au métier | `data-modele/` en entier |
...
```bash
# le site répond partout
for u in / /a-propos /nos-prestations /realisations /contact /mentions-legales; do
  printf '%-24s %s\n' "$u" "$(curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:8081$u)"
done
...
dossier de contenu et une table de pages — et la qualité vient des trois
