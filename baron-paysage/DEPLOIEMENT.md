...
├── baron-paysage/         ← le projet complet
│   ├── app/
│   ├── config/
│   ├── data/
...
- sinon, utilisez la **recherche** en bas de l'écran : tapez « Baron Paysage,
...
2. **Relisez.** Une traduction automatique se trompe souvent sur les noms
   propres — « Baron Paysage » ne doit pas être traduit. Corrigez
...
```bash
cd ~
git clone --branch claude/jardins-baron-redesign-86rfw2 \
  https://github.com/ftholomier/poubelle.git baron-paysage
cd baron-paysage
mkdir -p storage/cache data/admin
```

Puis pointez la racine du document du domaine sur `baron-paysage/public`
(section 1).
...
```bash
cd ~/baron-paysage && test -f app/bootstrap.php && \
git clone --branch claude/jardins-baron-redesign-86rfw2 \
