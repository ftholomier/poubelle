...
export const NAME_PATTERNS: Record<string, NameFn[]> = {
  ...
  restaurant: [
    (r) => `La Table de ${r.pick(FIRSTNAMES)}`,
    (r) => `Restaurant du ${r.pick(PLACES)}`,
    (r) => `Le ${r.pick(['Comtois', 'Saint-Laurent', 'Relais du ' + r.pick(PLACES), 'Petit Gourmet', 'Cerisier'])}`,
  ],
