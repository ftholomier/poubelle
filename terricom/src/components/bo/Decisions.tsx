...
function Msg({ state }: { state: ModState }) {
  ...
  return (
    <div className={`alert ${state.status === 'error' ? 'alert-error' : 'alert-ok'}`} role={state.status === 'error' ? 'alert' : 'status'} style={{ width: '100%' }}>
      {state.message}
    </div>
  );
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid var(--line-2)', paddingTop: 16 }}>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <form action={approveAction}>
          ...
          <button type="submit" disabled={approving} style={{ border: 0, background: 'var(--green)', color: '#fff', padding: '12px 20px', borderRadius: 12, fontWeight: 800, cursor: 'pointer' }}>
        ...
        <button
          ...
          style={{ border: '1.5px solid var(--ink)', background: 'transparent', padding: '12px 18px', borderRadius: 12, fontWeight: 700, cursor: 'pointer', color: 'var(--text)' }}
        >
        ...
        <button
          ...
          style={{ border: 0, background: 'var(--danger-bg)', color: 'var(--danger-fg)', padding: '12px 18px', borderRadius: 12, fontWeight: 700, cursor: 'pointer', marginLeft: 'auto' }}
        >
          Refuser
        </button>
      </div>
      {mode === 'info' ? (
        <form action={infoAction} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <textarea id="info-note" name="note" className="input" rows={3} required defaultValue="Pouvez-vous nous transmettre un extrait Kbis de moins de 3 mois, ou tout document attestant de votre lien avec l'établissement ?" />
      ...
      {mode === 'reject' ? (
        <form action={rejectAction} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <textarea id="reject-note" name="note" className="input" rows={3} required placeholder="Ex. : le SIRET fourni ne correspond pas à cet établissement." />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid var(--line-2)', paddingTop: 16 }}>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <form action={okAction}>
          ...
          <button type="submit" disabled={publishing} style={{ border: 0, background: 'var(--green)', color: '#fff', padding: '12px 20px', borderRadius: 12, fontWeight: 800, cursor: 'pointer' }}>
      ...
      {rejectOpen ? (
        <form action={koAction} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <textarea id="post-note" name="note" className="input" rows={3} required placeholder="Ex. : les promotions de plus de 50 % doivent préciser leur durée." />
          <button type="submit" className="btn btn-danger btn-sm" disabled={rejecting} style={{ alignSelf: 'flex-start' }}>
            Confirmer le refus
          </button>
        </form>
