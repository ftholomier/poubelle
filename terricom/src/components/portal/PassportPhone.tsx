    ...
    <div className="phone-mock">
      ...
      >
        ...
        <div style={{ background: 'var(--brand)', color: '#fff', padding: '48px 20px 20px' }}>
          <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.08em', color: 'var(--amber)' }}>{t('pp.title')}</div>
          ...
          <div style={{ fontSize: 12, marginTop: 6, color: 'var(--mint-3)' }}>{t('pp.stamps', { done, n: stops.length })}</div>
        </div>
          ...
          {stops.map((s, i) => {
            ...
            return demo ? (
              <button
                ...
                onClick={() => stamp(s.id)}
                disabled={pending}
                title={t('pp.demoTitle', { n: i + 1 })}
                aria-label={t(on ? 'pp.unstampAria' : 'pp.stampAria', { n: i + 1, name: s.name })}
                style={{ ...style, cursor: 'pointer', font: 'inherit', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 11 }}
              >
                {label}
              </button>
            ) : (
              <div key={s.id} style={style} aria-label={t(on ? 'pp.stepDone' : 'pp.stepTodo', { n: i + 1 })}>
                {label}
              </div>
            );
          })}
        ...
        <div style={{ margin: 'auto 16px 18px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ background: 'var(--amber)', borderRadius: 16, padding: 14, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {rewardCode ? (
              ...
              <>
                <div style={{ fontWeight: 800, fontSize: 14, color: 'var(--ink)' }}>{reward ?? t('pp.rewardDefault', { n: threshold })}</div>
                <div style={{ fontSize: 12, color: 'var(--amber-fg-2)' }}>{demo ? t('pp.demoHint') : t('pp.scanHint')}</div>
              </>
            )}
          </div>
        </div>
