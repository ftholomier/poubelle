...
export default async function ProfilePage() {
  ...
  return (
    <div className="app-content" style={{ maxWidth: 820 }}>
      <section className="card" style={{ borderRadius: 20, padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
      ...
      <section className="card" style={{ borderRadius: 20, padding: 24, marginTop: 16, display: 'flex', flexDirection: 'column', gap: 10, fontSize: 14 }}>
        <h2 className="display" style={{ fontSize: 22, margin: 0 }}>
          Vos accès
        </h2>
        ...
        {actor.memberships.length ? (
          <div>
            {actor.memberships.length} entreprise{actor.memberships.length > 1 ? 's' : ''} gérée{actor.memberships.length > 1 ? 's' : ''} (
            {actor.memberships.filter((m) => m.role === 'OWNER').length} en tant que titulaire).
          </div>
        ) : null}
      </section>
    </div>
