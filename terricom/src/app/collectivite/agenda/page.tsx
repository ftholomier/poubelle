...
export default async function AgendaPage({ searchParams }: Props) {
  ...
  if (tab === 'actualites') {
    const list = await db
      .select({ id: posts.id, title: posts.title, body: posts.body, status: posts.status, publishedAt: posts.publishedAt, publishAt: posts.publishAt, communeId: posts.communeId, communeName: communes.name })
      .from(posts)
      .leftJoin(communes, eq(communes.id, posts.communeId))
      .where(and(eq(posts.territoryId, ctx.territory.id), isNull(posts.establishmentId), ne(posts.status, 'ARCHIVED'), ctx.communeIds ? inArray(posts.communeId, ctx.communeIds) : undefined))
      ...
      <div className="app-content">
        ...
        <div className="split" style={{ ['--cols' as string]: 'minmax(0,1fr) minmax(0,1fr)', ['--gap' as string]: '18px', ['--align' as string]: 'start' }}>
          <section className="bo-card" style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            {list.map((p) => (
              <div key={p.id} style={{ display: 'flex', gap: 10, justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderTop: '1px solid var(--line-2)', fontSize: 14 }}>
                <Link href={`/collectivite/agenda?onglet=actualites&id=${p.id}`} style={{ color: 'var(--text)', minWidth: 0 }}>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--muted)' }}>
                    {p.status === 'SCHEDULED' && p.publishAt ? `programmée le ${fmtShortDate(p.publishAt)}` : p.publishedAt ? `publiée le ${fmtShortDate(p.publishedAt)}` : p.status.toLowerCase()}
          ...
          <section className="bo-card">
            ...
            <ActionForm key={editing?.id ?? 'new'} action={saveNewsAction} resetOnSuccess={!editing} style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 12 }}>
              <input type="hidden" name="postId" value={editing?.id ?? ''} />
              <input name="title" className="input" placeholder="Titre (ex. Travaux rue Courbet : les commerces restent ouverts)" defaultValue={editing?.title ?? ''} required aria-label="Titre" />
              <textarea name="body" className="input" rows={6} placeholder="Texte de l’actualité" defaultValue={editing?.body ?? ''} required aria-label="Texte" />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {communeSelect('communeId', editing?.communeId ?? null)}
                <input name="publishAt" type="date" className="input" aria-label="Date de publication (facultatif)" title="Laisser vide pour publier maintenant" />
              </div>
  ...
  if (tab === 'evenements') {
    const list = await db
      ...
      .where(and(eq(events.territoryId, ctx.territory.id), eq(events.status, 'PUBLISHED'), gte(events.startsAt, daysAgoDate(1)), ctx.communeIds ? inArray(events.communeId, ctx.communeIds) : undefined))
      ...
      <div className="app-content">
        ...
        <div className="split" style={{ ['--cols' as string]: 'minmax(0,1fr) minmax(0,1fr)', ['--gap' as string]: '18px', ['--align' as string]: 'start' }}>
          <section className="bo-card" style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            {list.map((ev) => (
              <div key={ev.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 10, padding: '10px 0', borderTop: '1px solid var(--line-2)', fontSize: 14, alignItems: 'center' }}>
                <div style={{ minWidth: 0 }}>
                  <span style={{ fontSize: 11, fontWeight: 800, padding: '2px 8px', borderRadius: 999, background: EVENT_KINDS[ev.kind as EventKind].bg, marginRight: 6 }}>{EVENT_KINDS[ev.kind as EventKind].label}</span>
          ...
          <section className="bo-card">
            ...
            <ActionForm key={editing?.id ?? 'new'} action={saveEventAction} resetOnSuccess={!editing} style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 12 }}>
              <input type="hidden" name="eventId" value={editing?.id ?? ''} />
              <input name="title" className="input" placeholder="Titre (ex. Marché de Noël d’Ornans)" defaultValue={editing?.title ?? ''} required aria-label="Titre" />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <input name="locationName" className="input" placeholder="Lieu (ex. Place Courbet)" defaultValue={editing?.locationName ?? ''} aria-label="Lieu" />
                ...
                <input name="organizerName" className="input" placeholder={`Organisateur (${ctx.scopeName})`} defaultValue={editing?.organizerName ?? ''} aria-label="Organisateur" />
              </div>
              <input name="summary" className="input" placeholder="Résumé en une phrase" defaultValue={editing?.summary ?? ''} maxLength={300} aria-label="Résumé" />
              <textarea name="description" className="input" rows={4} placeholder="Description" defaultValue={editing?.description ?? ''} required aria-label="Description" />
              <input name="registrationUrl" className="input" placeholder="Lien d’inscription (facultatif)" defaultValue={editing?.registrationUrl ?? ''} aria-label="Lien d’inscription" />
      ...
      <div className="split" style={{ ['--cols' as string]: 'minmax(0,1fr) minmax(0,1fr)', ['--gap' as string]: '18px', ['--align' as string]: 'start' }}>
        <section className="bo-card" style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {list.map(({ m, communeName }) => (
            <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 10, padding: '10px 0', borderTop: '1px solid var(--line-2)', fontSize: 14, opacity: m.isActive ? 1 : 0.55 }}>
        ...
        <section className="bo-card">
          ...
          <ActionForm key={editing?.id ?? 'new'} action={saveMarketAction} resetOnSuccess={!editing} style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 12 }}>
            ...
            <textarea name="description" className="input" rows={3} placeholder="Producteurs présents, spécialités…" defaultValue={editing?.description ?? ''} aria-label="Description" />
            <SubmitButton className="btn btn-brand" pendingLabel="Enregistrement…" style={{ alignSelf: 'flex-start' }}>
