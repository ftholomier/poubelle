...
export async function createCircuitAction(form: FormData): Promise<void> {
  ...
  const name = z.string().trim().min(3).max(200).catch('Nouveau circuit').parse(form.get('name') || 'Nouveau circuit');
  ...
  const [c] = await db
    .insert(circuits)
    .values({ territoryId: ctx.territory.id, slug, name, status: 'DRAFT', rewardThreshold: 5, rewardText: 'Un panier de produits locaux à gagner', createdById: ctx.actor.user.id })
    .returning({ id: circuits.id });
  await audit({ actor: { user: ctx.actor.user }, category: 'MODIFICATION', action: 'circuit.create', summary: `Circuit « ${name} » créé`, territoryId: ctx.territory.id, targetType: 'circuit', targetId: c.id });
  redirect(page(c.id));
}
...
export async function addStopAction(form: FormData): Promise<void> {
  ...
  if (!c) return;
  const [e] = await db.select({ id: establishments.id }).from(establishments).where(and(estScope(ctx), eq(establishments.id, estId))).limit(1);
  if (!e) return;
  const [dup] = await db.select({ id: circuitStops.id }).from(circuitStops).where(and(eq(circuitStops.circuitId, c.id), eq(circuitStops.establishmentId, e.id))).limit(1);
  if (dup) return;
  const [{ m }] = await db.select({ m: max(circuitStops.position) }).from(circuitStops).where(eq(circuitStops.circuitId, c.id));
...
export async function searchStopAction(_prev: CircState, form: FormData): Promise<CircState> {
  ...
  const [e] = await db
    ...
    .from(establishments)
    .where(and(estScope(ctx), sql`${establishments.status} in ('CLAIMED','VALIDATED','TO_COMPLETE','PRECREATED')`, sql`f_unaccent(${establishments.name}) ilike f_unaccent(${`%${q}%`})`))
    .limit(1);
...
export async function moveStopAction(form: FormData): Promise<void> {
  ...
  if (!c) return;
  const stops = await db.select({ id: circuitStops.id, position: circuitStops.position }).from(circuitStops).where(eq(circuitStops.circuitId, c.id)).orderBy(asc(circuitStops.position));
  const i = stops.findIndex((s) => s.id === stopId);
  if (i <= 0) return;
  await db.update(circuitStops).set({ position: stops[i - 1].position }).where(eq(circuitStops.id, stops[i].id));
  await db.update(circuitStops).set({ position: stops[i].position }).where(eq(circuitStops.id, stops[i - 1].id));
  revalidatePath('/collectivite/circuits');
}
...
export async function saveCircuitAction(_prev: CircState, form: FormData): Promise<CircState> {
  ...
  if (d.status === 'PUBLISHED') {
    const [{ n }] = await db.select({ n: sql<number>`count(*)::int` }).from(circuitStops).where(eq(circuitStops.circuitId, c.id));
  ...
  await audit({ actor: { user: ctx.actor.user }, category: 'MODIFICATION', action: 'circuit.update', summary: `Circuit « ${c.name} » mis à jour (${d.status.toLowerCase()})`, territoryId: ctx.territory.id, targetType: 'circuit', targetId: c.id });
  revalidatePath('/collectivite/circuits');
