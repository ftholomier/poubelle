...
const MONTH_NAMES: Record<string, string> = { jan: 'janvier', fév: 'février', mar: 'mars', avr: 'avril', mai: 'mai', juin: 'juin', juil: 'juillet', août: 'août', sep: 'septembre', oct: 'octobre', nov: 'novembre', déc: 'décembre' };
    ...
    <div className="app-content">
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        {(Object.keys(PERIODS) as (keyof typeof PERIODS)[]).map((p) => (
          <Link key={p} href={`/collectivite/statistiques${q({ periode: p === '12m' ? null : p })}`} className="bo-chip" aria-current={p === period ? 'true' : undefined}>
            {PERIODS[p].label}
          </Link>
        ))}
        {ctx.level === 'TERRITORY' ? (
        ...
        <a href={`/api/collectivite/statistiques.csv${exportQs}`} className="btn btn-outline btn-sm" style={{ border: '1.5px solid var(--ink)', color: 'var(--ink)' }}>
          Export CSV
        </a>
      </div>
      ...
      <div className="split" style={{ ['--cols' as string]: 'minmax(0,1.6fr) minmax(0,1fr)', ['--gap' as string]: '18px' }}>
        <section className="bo-card" style={{ padding: 20, borderRadius: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14, gap: 10 }}>
            ...
            {s.launchedLabel ? <span style={{ fontSize: 12, color: 'var(--muted)' }}>lancement pilote : {MONTH_NAMES[s.launchedLabel] ?? s.launchedLabel}</span> : null}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12,1fr)', gap: 8, alignItems: 'end', height: 220 }} role="img" aria-label="Histogramme des visiteurs mensuels">
            {s.months.map((m, i) => (
        ...
        <section className="bo-card" style={{ padding: 20, borderRadius: 20, display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {s.searches.map((t, i) => (
            <div key={t.q} style={{ display: 'grid', gridTemplateColumns: '22px 1fr auto', gap: 8, padding: '8px 0', borderTop: '1px solid var(--line-2)', fontSize: 14 }}>
          ...
          {s.signal ? (
            <div style={{ marginTop: 10, background: 'var(--amber)', borderRadius: 12, padding: 12, fontSize: 13, color: 'var(--ink)' }}>
              <b>Signal faible :</b> {fmtInt(s.signal.n)} recherches « {s.signal.q} », aucun référencé sur le territoire. Une piste pour l&apos;installation d&apos;une nouvelle activité ?
            </div>
          ) : null}
        </section>
      </div>
      ...
      <section style={{ background: 'var(--paper)', border: '1px solid var(--line)', borderRadius: 20, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <div style={{ minWidth: 640 }}>
            <div className="bo-table-head" style={{ display: 'grid', gridTemplateColumns: '1.6fr repeat(5,1fr)', gap: 12, padding: '12px 18px', borderBottom: '1px solid var(--line)' }}>
              <span>Commune</span>
              <span>Fiches</span>
            ...
            {communeRows.map((c) => (
              <div key={c.id} style={{ display: 'grid', gridTemplateColumns: '1.6fr repeat(5,1fr)', gap: 12, padding: '11px 18px', fontSize: 14, borderBottom: '1px solid var(--line-3)' }}>
                <b>{c.name}</b>
