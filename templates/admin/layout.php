...
$entries = [
    ...
    ['/admin/assistant',       'admin.assistant',    '#FF4B3E'],
...
// Rubriques à plusieurs écrans : l'entrée reste allumée sur chacun.
$within = ['/admin/assistant' => ['/admin/documents']];
?>
<!DOCTYPE html>
...
<?php // Empreinte de fichier, comme sur le site : les feuilles sont en cache un an. ?>
    ...
    <div class="admin-grid">
      <nav class="admin-nav" aria-label="<?= e(I18n::t('admin.dashboard')) ?>">
        <?php foreach ($entries as [$href, $key, $color]): ?>
          <a href="<?= e($href) ?>"<?= $current === $href || in_array($current, $within[$href] ?? [], true) ? ' aria-current="page"' : '' ?>>
